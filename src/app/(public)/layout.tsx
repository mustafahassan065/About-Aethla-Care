...
const portals = [
  ...
  },
  {
    key: 'patient',
    label: 'Patient',
    desc: 'Patients, guardians & family',
    href: '/portal/login',
    color: '#C9A84C',
  },
]
          ...
          <div className="flex items-center justify-between h-[76px]">
            ...
            <div className="hidden lg:flex items-center gap-6">
              ...
              <div className="relative"
                ...
                <button className={`flex items-center gap-1 text-body-sm font-medium transition-colors ${
                    ...
                    <motion.div
                      ...
                      {portals.map((p) => (
                        <Link key={p.key} href={p.href}
                          className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-neutral-50 transition-all group">
                          <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
                            style={{ background: `${p.color}18`, border: `1.5px solid ${p.color}30` }}>
                            <div className="w-3 h-3 rounded-full" style={{ background: p.color }} />
                          </div>
                          <div>
                            <p className="text-body-sm font-bold font-poppins text-neutral-800 group-hover:text-primary-500">
                              {p.label}
                            </p>
            ...
            {/* Mobile toggle */}
            <button className="lg:hidden p-2 rounded-lg text-neutral-700 hover:bg-neutral-100 transition-colors"
        ...
        {mobileOpen && (
          <motion.div
            ...
            <div className="flex flex-col p-6 gap-1">
              ...
              {/* Portal Access Section in Mobile */}
              <div className="mt-4 pt-4 border-t border-neutral-100">
                <p className="text-overline text-neutral-400 mb-3 px-3">Portal Access</p>
                {portals.map((p) => (
                  <Link key={p.key} href={p.href}
                    className="flex items-center gap-3 py-3 px-3 rounded-xl mb-1 transition-all"
              ...
              <div className="mt-4 flex flex-col gap-3">
                <a href={`https://wa.me/${process.env.NEXT_PUBLIC_WHATSAPP_NUMBER}`}
                  ...
                  className="btn-outline text-center py-3 flex items-center justify-center gap-2">
                  <MessageCircle size={16} /> WhatsApp Support
                </a>
                <Link href="/contact" className="btn-primary text-center py-3">Book Free Consultation</Link>
              </div>

              <div className="mt-4 p-4 rounded-2xl" style={{ background: 'var(--color-primary-light)' }}>
...
function NavLink({ href, label, pathname }: { href: string; label: string; pathname: string }) {
  ...
  return (
    <Link href={href} className={`text-body-sm font-medium transition-colors ${
