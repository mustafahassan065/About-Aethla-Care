...
export default function PatientPortalPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!signupForm.accountType) {
      setError('Please select your account type.')
      return
    }
    if (!signupForm.isSelf && !signupForm.relationship) {
      ...
      <div className="min-h-screen flex items-center justify-center px-4"
        ...
        <div className="bg-white rounded-3xl p-10 shadow-2xl w-full max-w-md text-center">
          ...
          <p className="text-body-md text-neutral-500 mb-2">
            Your account request has been received and is pending approval from our care coordinator.
          </p>
          <p className="text-body-sm text-neutral-400 mb-6">
            You will receive an email at <strong>{signupForm.email}</strong> once your account has been approved and activated.
          </p>
          <p className="text-body-sm text-neutral-500">
            For urgent assistance, call us on{' '}
            <a href="tel:+97440000000" className="text-primary-500 font-semibold hover:underline">+974 4000 0000</a>
          </p>
          <button onClick={() => { setSuccess(false); setTab('login') }}
            className="btn-outline btn-sm mt-6">Back to Login</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-8"
      ...
      <div className="relative w-full max-w-md">
          ...
          <div className="flex border-b border-neutral-100">
            {(['login', 'signup'] as Tab[]).map(t => (
              <button key={t} onClick={() => { setTab(t); setError('') }}
                className={`flex-1 py-4 text-body-sm font-semibold capitalize transition-all ${
                  tab === t
            ...
            {tab === 'login' && (
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <p className="text-caption text-neutral-400 text-center">
                  No account yet?{' '}
                  <button type="button" onClick={() => setTab('signup')} className="text-primary-500 hover:underline font-semibold">
                    Create one here
                  </button>
                </p>
              </form>
              ...
              <form onSubmit={handleSignup} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    {accountTypes.map(t => (
                      <button key={t.value} type="button"
                        onClick={() => {
                          setSignup('accountType', t.value)
                          setSignup('isSelf', t.value === 'patient')
                        }}
                        className={`p-3 rounded-xl border text-left transition-all ${
                          signupForm.accountType === t.value
                            ? 'border-primary-500 bg-primary-50'
                            : 'border-neutral-200 hover:border-neutral-300'
                        }`}>
                        <p className={`text-body-sm font-bold font-poppins ${
                          signupForm.accountType === t.value ? 'text-primary-500' : 'text-neutral-700'
                        }`}>{t.label}</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input value={signupForm.lastName} onChange={e => setSignup('lastName', e.target.value)}
                      className="form-input" placeholder="Al-Rashid" />
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <input value={signupForm.patientLastName} onChange={e => setSignup('patientLastName', e.target.value)}
                          className="form-input" placeholder="Family name" />
                      </div>
                    </div>
                    <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="tel" value={signupForm.phone} onChange={e => setSignup('phone', e.target.value)}
                      className="form-input pl-10" placeholder="+974 5500 0000" />
                  </div>
                </div>
                ...
                <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
                  <p className="text-caption text-amber-700">
                    Your account will be reviewed and activated by our care coordinator within 24 hours.
                ...
                <p className="text-caption text-neutral-400 text-center">
                  ...
                  <button type="button" onClick={() => setTab('login')} className="text-primary-500 hover:underline font-semibold">
                    Sign in here
                  </button>
                </p>
              </form>
        ...
        <p className="text-center text-white/40 text-caption mt-5">
          Need help?{' '}
          <a href="tel:+97440000000" className="text-white/60 hover:text-white">+974 4000 0000</a>
        </p>
      </div>
    </div>
  )
