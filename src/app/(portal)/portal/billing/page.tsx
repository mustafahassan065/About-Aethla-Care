...
export default function PortalBilling() {
  ...
  const outstanding = invoices.filter((i: any) => ['sent', 'overdue'].includes(i.status)).reduce((s: number, i: any) => s + (i.total || 0), 0)

  if (!client) {
    return (
      <div><PageHeader title="Billing" subtitle="Your invoices and payment history" />
        <div className="card p-12 text-center"><p className="text-body-md text-neutral-500">Account not yet linked. Contact your care coordinator.</p></div>
      </div>
    )
  }
  ...
  return (
    <div>
      <PageHeader title="Billing" subtitle={`Invoices for ${client.firstName} ${client.lastName}`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <div className="card p-5" style={{ borderLeft: '4px solid #F59E0B' }}>
          ...
          <p className="text-2xl font-extrabold font-poppins text-neutral-800">QAR {outstanding.toLocaleString()}</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="card p-16 text-center"><p className="text-body-md text-neutral-400">No invoices yet.</p></div>
      ) : (
        <div className="card overflow-hidden">
          <table className="data-table">
            <thead>
              <tr><th>Invoice</th><th>Amount (QAR)</th><th>Due Date</th><th>Status</th></tr>
            </thead>
            <tbody>
              {invoices.map((inv: any) => (
                <tr key={inv._id}>
                  ...
                  <td><p className="text-body-sm font-bold font-poppins text-neutral-800">QAR {(inv.total || 0).toLocaleString()}</p></td>
                  <td><p className="text-body-sm text-neutral-600">{inv.dueDate || '—'}</p></td>
      ...
      <div className="card p-5 mt-4">
        <p className="text-body-sm text-neutral-500">
          For billing queries contact your care coordinator on{' '}
